...
async function init() {
  try {
    const result = await tracedFetch(
      'https://distribution.classisland.tech/api/v1/public/distributions/web'
    );
    ...
    <div v-else-if="!isError" class="d-flex flex-column mt-8">
      ...
      <div class="d-flex">
        <v-alert
          ...
          icon="mdi-bullhorn"
          color="blue-lighten-3"
          text="ClassIsland 2 正式版 已经发布，但如您仍有需求，也可下载 ClassIsland 1。"
        >
          <template v-slot:append>
            <v-btn variant="text" style="margin: -8px 0" to="/download/v1">前往下载</v-btn>
          </template>
        </v-alert>
      </div>
      <div
        ...
        >
          <div class="d-flex flex-row flex-wrap align-center justify-center mt-2 ga-1">
            <SplitDownloadButton
              ...
              :download-route-root="
                '/download/thank_you/v2/' + latestVersionInfo.latestVersionId + '/'
              "
            />
      ...
      <div class="bottom-operations">
        <div />
        <div
          class="d-flex flex-row flex-wrap ga-4 justify-center align-self-center align-content-center"
        >
          <v-btn
            color="blue-lighten-3"
            variant="text"
