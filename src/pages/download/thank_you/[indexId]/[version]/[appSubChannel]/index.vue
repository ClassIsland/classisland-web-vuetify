  ...
  try {
    ...
    if (subChannel == null) {
      console.error(
        ...
        appSubChannel.toString(),
        'Available:',
        Object.keys(latestVersionInfo.value.DownloadInfos)
      );
      isError.value = true;
      isLoading.value = false;
      return;
    ...
    <div v-show="!isLoading && !isError" class="flex-column mt-12">
      <div class="page-margin-x">
        <h2 class="align-self-center text-center mb-6 text-h2 font-weight-bold download-main-title">
          感谢下载 ClassIsland {{ latestVersionInfo.Title }} ！
        </h2>
        <p class="text-center align-self-center mb-16">
          您的下载即将开始。如果下载在 30 秒内没有开始，请点击<a
            :href="selectedSubChannel.ArchiveDownloadUrls.main"
            id="downloadLink"
            :download="'ClassIsland_' + appSubChannel + '_' + version + '.zip'"
            >此处</a
          >手动开始下载。
        </p>
        ...
        <v-text-field
          ...
          variant="outlined"
          readonly
          v-model="selectedSubChannel.ArchiveDownloadUrls.main"
        ...
        <v-text-field
          ...
          variant="outlined"
          readonly
          v-model="selectedSubChannel.ArchiveSHA256"
          append-icon="mdi-content-copy"
        ...
        <v-alert type="warning" variant="outlined" class="mt-4 mb-4" color="orange-lighten-1">
          ...
          <p class="mb-1">
            • 下载的压缩包内的 <code>ClassIsland.exe</code>
            <strong>不是安装程序</strong>，而是直接运行的应用程序
          </p>
          <p class="mb-1">
            • 请将程序解压到一个<strong>有读写权限的文件夹</strong>中（建议避免中文路径）
          </p>
      ...
      <div class="page-margin-x py-4 mt-8">
        ...
        <div class="d-flex flex-row my-4 ga-6 flex-wrap">
          <div class="setup-step">
            ...
            <h3>第一步：解压应用</h3>
            <p>
              在一个合适的地方（比如 D 盘等）新建一个文件夹，将下载的压缩包中的文件解压到文件夹中。
            </p>
        ...
        <v-expansion-panels class="mt-4">
          <v-expansion-panel title="🤔 下载后找不到安装程序？">
            <v-expansion-panel-text>
              ClassIsland 是便携版软件，<strong>没有安装程序</strong>。下载后解压 zip 文件，直接运行
              <code>ClassIsland.exe</code> 即可使用。
            </v-expansion-panel-text>
          </v-expansion-panel>
          ...
          <v-expansion-panel title="📁 建议放在哪个文件夹？">
            <v-expansion-panel-text>
              推荐放置位置：
              <br />• <code>D:\ClassIsland\</code>（推荐） <br />•
              <code>C:\Users\你的用户名\ClassIsland\</code> <br /><strong>避免放在：</strong
              >桌面、下载文件夹、含中文路径的位置
            </v-expansion-panel-text>
          </v-expansion-panel>
