...
async function init() {
  try {
    console.log('Route params:', {
      ...
      appSubChannel: appSubChannel.toString()
    });

    const result = await tracedFetch(
      `https://distribution.classisland.tech/api/v1/public/distributions/web/${version}/${appSubChannel}`
    );
  ...
  <div class="d-flex download-container flex-column">
    ...
    <div v-show="!isLoading && !isError" class="flex-column mt-12">
      <div class="page-margin-x">
        <h2 class="align-self-center text-center mb-6 text-h2 font-weight-bold download-main-title">
          感谢下载 ClassIsland {{ downloadInfo.version }} ！
        </h2>
        <p class="text-center align-self-center mb-16">
          您的下载即将开始。如果下载在 30 秒内没有开始，请点击<a
            :href="downloadInfo.archiveUrl"
            id="downloadLink"
            :download="'ClassIsland_' + appSubChannel + '_' + downloadInfo.version + '.zip'"
            >此处</a
          >手动开始下载。
        </p>
        ...
        <v-text-field
          ...
          variant="outlined"
          readonly
          v-model="downloadInfo.archiveUrl"
          append-icon="mdi-content-copy"
          @click:append="copyDownloadUrl"
        />
        <v-text-field
          label="校验和（SHA512）"
          variant="outlined"
          readonly
          v-model="downloadInfo.archiveSHA512"
          append-icon="mdi-content-copy"
          @click:append="copyChecksum"
        />

        <p>请注意核对文件 SHA512 是否正确。</p>

        <v-alert type="info" variant="outlined" class="mb-4 mt-4" color="cyan-lighten-3">
          <p v-if="scPackage == 'folder'">
            您下载的是便携版 ClassIsland，<b
              >需要完整解压<span v-if="scBuildType == 'full'">并按照指示安装运行时</span></b
            >后才能使用。
            <span v-if="scBuildType == 'full'"
              >如果您要安装本应用的设备有自动还原功能，我们建议您下载含运行时的版本，以免去在每次系统还原后重新安装运行时的麻烦。</span
            >
          </p>
          <p v-else-if="scPackage == 'installer'">
            您下载文件是 ClassIsland 安装程序，您需要先运行此安装程序才能使用 ClassIsland。
          </p>
          <p v-else-if="scPackage == 'deb' || scPackage == 'pkg'">
            您下载文件是 ClassIsland 安装包，您需要先安装此安装包才能使用 ClassIsland。
          </p>
        </v-alert>
      </div>
      ...
      <div class="page-margin-x py-4 mt-8">
        ...
        <div class="d-flex flex-row my-4 ga-6 flex-wrap">
          <div class="setup-step" v-if="scPackage == 'folder'">
            <v-img
              v-if="scOs == 'windows'"
              src="../../../../../../assets/setup/windows/1.png"
              class="mb-4"
            />
            <v-img
              v-if="scOs == 'linux'"
              src="../../../../../../assets/setup/linux/6.png"
              class="mb-4"
            />
            <h3>第一步：解压应用</h3>
            <p>
              在一个合适的地方<span v-if="scOs == 'windows'">（比如 D 盘等）</span
              ><span v-else>（比如用户目录等）</span
              >新建一个文件夹，将下载的压缩包中的文件解压到文件夹中。
            </p>
          ...
          <div class="setup-step" v-else>
            <v-img
              v-if="scOs == 'macos'"
              src="../../../../../../assets/setup/macos/7.png"
              class="mb-4"
            />
            <v-img
              v-if="scOs == 'linux'"
              src="../../../../../../assets/setup/linux/4.png"
              class="mb-4"
            />
            <h3>第一步：安装应用</h3>
          ...
          <div class="setup-step" v-if="scBuildType == 'full'">
            <v-img
              v-if="scOs == 'windows'"
              src="../../../../../../assets/setup/windows/2.png"
              class="mb-4"
            />
            <h3>第二步：安装 .NET 运行时</h3>
            <p>
              运行应用，如果您的设备没有安装 .NET 运行时，可以根据启动应用后的弹窗的指示安装 .NET
              运行时。
            </p>
          </div>
          <div class="setup-step">
            <v-img
              v-if="scOs == 'windows'"
              src="../../../../../../assets/setup/windows/3.png"
              class="mb-4"
            />
            <v-img
              v-if="scOs == 'linux'"
              src="../../../../../../assets/setup/linux/5.png"
              class="mb-4"
            />
            <v-img
              v-if="scOs == 'macos'"
              src="../../../../../../assets/setup/macos/8.png"
              class="mb-4"
            />
            <h3 v-if="scBuildType == 'full'">第三步：开始使用</h3>
            <h3 v-else>第二步：开始使用</h3>
            <p v-if="scBuildType == 'full'">
              完成上述步骤后，再次运行应用，根据应用弹出的欢迎向导即可完成应用设置。
            </p>
        ...
        >
          ...
          <p v-if="scBuildType == 'full'" class="mb-0">
            • 由于您下载的版本不包含运行时，您在学校安装本应用时可能需要一并安装 .NET 桌面运行时
          </p>
        </v-alert>
        <p>
          更详细的安装说明请参见文档<a
            href="https://docs.classisland.tech/app/setup.html"
            target="_blank"
            >安装与开始</a
          >。
        </p>
      ...
      <v-sheet class="elevated-sheet">
        <div class="page-margin-x py-4 mt-8">
          ...
          <p>
            使用过其它课表软件？您可以使用 CSES 或
            <a href="https://migrate.classisland.tech/" target="_blank">ClassIsland 迁移向导</a
            >快速从其它课表软件导入课表。
          </p>
        </div>
      </v-sheet>
        ...
        <v-expansion-panels class="mt-4">
          <v-expansion-panel title="⚡ 程序启动后提示安装 .NET？" v-if="scBuildType == 'full'">
            <v-expansion-panel-text>
              ...
              <br />• 点击安装按钮，程序会自动跳转到 .NET 官方网站下载安装程序 <br />•
              安装完成后重新运行 ClassIsland 即可
            </v-expansion-panel-text>
          </v-expansion-panel>
          ...
          <v-expansion-panel title="📁 建议放在哪个文件夹？" v-if="scPackage == 'folder'">
            <v-expansion-panel-text>
              ...
              <span v-if="scOs == 'windows'"
                >• <code>C:\Users\你的用户名\ClassIsland\</code><br
              /></span>
          ...
          <v-expansion-panel title="🎯 如何开始使用？">
            <v-expansion-panel-text>
              ...
              <br />2. <strong>强烈建议先观看上方的视频教程</strong> <br />3.
              在档案编辑器中添加您的课表和时间表 <br />4. 根据需要调整组件和插件
            </v-expansion-panel-text>
          </v-expansion-panel>
        </v-expansion-panels>
      </div>
      ...
      <div class="page-margin-x py-4 my-8">
        ...
        <div class="d-flex flex-row justify-center ga-2 flex-wrap mb-4">
          ...
          <v-btn
            ...
            href="https://docs.classisland.tech/community/"
            target="_blank"
            >加入讨论社区</v-btn
          >
        </div>
    ...
    <div v-if="!isLoading && isError" class="flex-column mt-12">
      <div class="page-margin-x">
        ...
        <p class="text-center align-self-center mb-16">
          找不到请求的下载信息。可能是您提供的链接有误，或下载服务器暂时不可用。
        </p>
    ...
    <v-snackbar v-model="showCopiedSnackbar"> 已复制到剪贴板。 </v-snackbar>
  </div>
</template>
